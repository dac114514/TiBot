# Subagent-Driven Development (TiBot 定制版)

> 在原版 superpowers/subagent-driven-development 基础上, 叠加 TiBot 项目规则
...
## TiBot 实施流程（针对 4 个项目 agent）

```
1. orchestrator 接收 plan
...
5. android-review 通过
   ↓
6. android-coder 做 version bump (CLAUDE.md 硬性)
   → app/build.gradle.kts: versionCode+1 (37→38), versionName 语义化 (2.2.2→2.2.3)
   → invoke superpowers/verification-before-completion (TiBot 化) 必查
   ↓
...
8. 派 android-build (, ask 权限 = 用户确认) 监控 CI
   ...
   ↓
9. invoke superpowers/finishing-a-development-branch (TiBot 化)
   → 完成, 报告用户
```
