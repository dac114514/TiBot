...
class ProotManager private constructor(private val context: Context) {
    ...
    suspend fun startProot(): Process? = withContext(Dispatchers.IO) {
        ...
        prootTmpDir.mkdirs()
        // Proot loader binaries at executable location (Android W^X workaround)
        pb.environment()["PROOT_LOADER"] = "/data/local/tmp/com.faster.tibot/loader"
        pb.environment()["PROOT_LOADER_32"] = "/data/local/tmp/com.faster.tibot/loader32"
